...
Três pontos não se fecham aqui e vão para a seção 11.
...
- **O identificador de correlação entre os dois lados do erro.** O ADR-0002 registrou que um erro passa a ter dois lados e que nada correlaciona os dois sozinho, e esta seção não resolve isso. Quem lê o registro do tier Front e quem lê o da API não tem como saber que as duas linhas são a mesma falha. O `traceId` da seção 9.3 nasce dentro da API e não cobre o salto 2. O lugar já existe no diagrama de camadas, que é o interesse transversal "Erros e log" ligado às três camadas, e não há nenhuma regra de log escrita em artefato nenhum do projeto.
...
Esta política supõe que o `personalToken` tenha pelo menos a entropia do `publicToken`. Ele não tem ADR, e a pendência 2 do Diagrama de Classes já pede entropia, alfabeto e revogação. Sem essa decisão, o colapso naquela rota não se sustenta.
...
**Como o endereço da API chega ao código que roda no navegador não está decidido.** Nenhum artefato do projeto escreve isso, e a página [Configuração de Ambiente](../Começando/Configuração-de-Ambiente.md), que seria o lugar, é hoje um marcador de uma linha. Fica na seção 10.7 com dono.
...
| Onde `SessionGuard` e `RateLimitGuard` guardam estado | Os itens 5 e 6 da seção 10.2 do Diagrama de Componentes pedem ADR próprio para isso | Um ADR que ainda não existe. Nenhuma das alternativas em discussão acrescenta container, então os tiers continuam três |
...
- **A projeção que `findAttendanceByStatus` devolve não tem nome.** É por isso que o quarto tipo de retorno da 5.2 não fecha. `CategoryCount`, `AllergyDescription` e `ExportRow` estão nomeados no `diagrama-sequencia-uc008.puml`, e a projeção da ED1 do UC007, com nome, status e número de acompanhantes, não está nomeada em lugar nenhum. Resolve: o diagrama de sequência do UC007, que não existe. Board: #62.
- **Onde as guardas de sessão e de limite de taxa guardam estado.** As pendências 5 e 6 da seção 10.2 do Diagrama de Componentes descrevem o problema e pedem ADR próprio. O que o guia acrescenta é que a escolha não é livre e não pode esperar a implementação. A leitura estrita da 6.2 proíbe a Apresentação de falar com o banco, então tabela de sessão ou tabela de contador no banco principal é violação direta da regra da seção 6, e a segunda busca da 6.4 a encontra em revisão. Contador em memória do processo não cria dependência nenhuma e deixa de valer se a API ganhar réplica. Resolve: ADR próprio, o 0010. Board: #63.
- **O `personalToken` do convidado não tem ADR.** A pendência 2 do Diagrama de Classes já registra a falta de entropia, alfabeto e revogação. O que é do guia é que a seção 3.2 já alocou `generatePersonalToken()` no Domínio e a 3.1 já proibiu a Apresentação de gerar identificador, ou seja, o guia decidiu onde o token nasce sem que exista decisão sobre o que ele é. `generatePublicToken()` tem o ADR-0005 atrás de si. Este não tem nada. Resolve: ADR próprio. Board: #63.
- **Se o tier Front repassa o endereço de origem do convidado para a API.** A linha 4 da 11.1 separou o limite de taxa em duas assinaturas, e a própria seção 4.1 registra que aquela decisão vale "até o time decidir se o tier Front repassa o endereço original". A pendência sustenta a decisão e não aparecia em lugar nenhum desta seção. No dia em que o Front repassar o endereço do convidado, as duas assinaturas voltam a ser uma e a linha 4 se desfaz sozinha, sem precisar de recusa do time. Enquanto isso não for decidido, a leitura pública conta todos os convidados de um convite no mesmo balde. Resolve: o time, junto com a linha correspondente da seção 10.7, que registra a mesma pendência pelo lado da topologia. Board: #63.
- **Fuso horário e fim do evento.** A pendência 4 do Diagrama de Classes registra que `Invite` tem data e hora separadas, sem duração e sem fuso. O que é do guia é que a tabela 9.1 já promete 422 `VALIDATION_FAILED` para data no passado, RN1 do UC002, e essa regra não tem definição no próprio dia da festa. Categoria de erro que não dá para implementar não é categoria. Resolve: a Especificação de Casos de Uso e o Diagrama de Classes. Board: #61 para o diagrama, e a especificação não tem item aberto na Sprint 2, então a correção precisa de item novo.
